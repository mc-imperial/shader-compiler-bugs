...
#define POS pos_vec.zw

void main(void)
{
    vec2 pos = (gl_FragCoord.xy * 2.0 - resolution.xy) / resolution.y;
    vec4 pos_vec = vec4(0.0, 0.0, pos);
    ...
    float fovy = 60.0;
    vec3 rayDir = normalize(camSide * - POS.x + camUp * - POS.y + camDir * 1.0 / tan(fovy * 0.5 * ((((injectionSwitch.x > injectionSwitch.y)) ? (1.0) : (3.14159265359 / 180.0)))));
    vec3 ray = camPos;
    float m = 0.0;
